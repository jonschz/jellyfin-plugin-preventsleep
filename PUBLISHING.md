...
5. Create a release on GitHub following the pattern of previous releases. Be sure to set the attributes (pre-release vs. latest) correctly.
6. Verify that the correct git tag was generated by the GitHub release process. Fix manually if needed.
7. Stable releases only: Add the new version to [manifest.json](./manifest.json), create a PR, and merge.
